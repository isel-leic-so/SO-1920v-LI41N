...
VOID UtExit (LONG res) {
	...
	// check if tere are no thread waiting for result
	if (IsListEmpty(&running_thread->res_waiters)) {
		// Save result in thread descriptor
		running_thread -> hasResult = TRUE;
		running_thread->res = res;
	}
	else {
		do {
			
			PRES_WAITBLOCK pblock = (PRES_WAITBLOCK) RemoveHeadList(&running_thread->res_waiters);
			pblock->result = res;
			
			UtActivate(pblock->threadHandle);
			
		}
		while(!IsListEmpty(&running_thread->res_waiters));
...
//
VOID internal_start () {
	LONG res = running_thread->Function(running_thread->Argument);
...
HANDLE UtCreate64 (UT_FUNCTION Function, UT_ARGUMENT Argument) {
	...
	// Initialize result waiters list
	InitializeListHead(&Thread->res_waiters);
...
BOOLEAN UtTryGetThreadResult(HANDLE ThreadHandle, LONG* result) {
	...
	// the running thread is not a terminated thread by definition
	if (UtSelf() == ThreadHandle) return FALSE;
	
	if (thread->hasResult) {	// setted in termination thread
		*result = thread->res;
		free(thread); // now the thread is completelly terminated
		return TRUE;
	}
	return FALSE;
}

static VOID initialize_res_wait_block(PRES_WAITBLOCK wait) {
	wait->threadHandle = UtSelf();
}
...
BOOLEAN UtGetThreadResult(HANDLE ThreadHandle, LONG* result) {
	...
	if (UtTryGetThreadResult(ThreadHandle, result)) return TRUE;
	
	PUTHREAD thread = (PUTHREAD) ThreadHandle;
	
	RES_WAITBLOCK wait_block;
	
	initialize_res_wait_block(&wait_block);
	
	InsertTailList(&thread->res_waiters, &wait_block.link);
	
	UtDeactivate();
	
	// retrieve the result saved in wait_block
	*result = wait_block.result;
